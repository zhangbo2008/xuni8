...
#=========== 设立一个全局变量, 所有用户共享.这样.就是一个私立的id对于每一次的访问
cnt=0
#=========触发函数要卸载界面的上面.
...
#         mutex.release()
        ...
        args2=args
        import os
        os.system(f'edge-tts --voice zh-CN-XiaoxiaoNeural --text "{args.text}" --write-media {wavfile}')
      ...
      #=======推理代码. 参数都已经设置好了,直接跑即可. 结果再results/result_voice.mp4里面.
        ...
        parser.add_argument('--outfile', type=str, help='Video path to save result. See default for an e.g.', 
                                        default='tmp/result99999999.mp4')
        ...
        args.nosmooth =True # ======越大速度越快. 基本500足够了.
        ...
        def datagen(frames, mels):
            ...
            print(22222222222222222222222,len(mels))
...
with gr.Blocks(title="ChatGLM2-6B-int8", theme=gr.themes.Soft(text_size="sm")) as demo:
        ...
        with gr.Column(scale=4):
            ...
            with gr.Column(min_width=32, scale=1):
                with gr.Row():
                    ...
                    btn = gr.Button(value="手动生成视频",visible=False)
    ...
    response2=gr.State(value='') # 设置成session界别共享变量.
    ...
    # aaa.autoplay=True
    # gr.aaa=aaa
    btn.click(combine,inputs=[history],outputs=[videostate])
...
demo.queue(concurrency_count=3, max_size=30).launch(server_name='0.0.0.0',debug=True,show_api=True,share=True)
